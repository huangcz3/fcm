...
function getPageData(){    //根据筛选条件，获取页面数据
    ...
    $(".modal").show();
    $.ajax({
        url:'./channelEvalution/getChannelReportInfo',
        data:param,
        dataType:"json",
        type:"get",
        caches:false,
        success:function(res){
            ...
            if(res.code == 0){
                ...
                fillTable(res.channelReportList);
                writeCache(res.channelReportList, 1);
                //分页
    ...
    $.ajax({
        ...
        success:function(res){
            ...
            if(res.code == 0){
                res = res.data;
                fillTable(res.channelReportList);
                writeCache(res.channelReportList, param.page);
            }else{
                showMessage(res.msg,"error");
            }
        },
...
$("#download").click(function(){
    ...
    param.filtered = $(".radio-box p input[checked]").val();
    param.page = 1;
    window.location.href = './channelEvalution/getChannelReportDownload?time='+param.time+'&areaId='+param.areaId+'&channelId='+param.channelId+'&filtered='+param.filtered;
});
...
$(document).ready(function(){
    ...
    sideNav.init();
    sideNav.show("#channelAssessment");
});
