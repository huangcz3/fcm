	...
	public List readExcel(InputStream is){
		...
		public static void checkExcelVaild(File file) throws Exception{
			...
			if(!(file.isFile() && file.getName().endsWith(EXCEL_XLSX))){
				throw new Exception("文件不是Excel或者不是XLSX");
			}
		}

		/**
		 * 读取Excel测试，兼容 Excel 2007/2010
		 * @throws Exception
		 */

//		public void readRedSeaExcel(File file) throws IOException {
		public static void main(String[] args) throws IOException, InvalidFormatException {
			SimpleDateFormat fmt = new SimpleDateFormat("yyyy-MM-dd");
			String filePath = "E:/红海行动各中心完成情况-（日报定报）价值组模板V2.xlsx";
			...
			for (int j = 3; j < LastCellNum; j++) {
				org.apache.poi.ss.usermodel.Cell cell = rownumbigger.getCell(j);
				...
				for (int i = 0; i < sheetMergeCount; i++) {
					...
					if (num >= firstRow && num <= lastRow) {
						...
						if (a >= firstColumn && a <= lastColumn) {
							...
							if (i==countRow ){
								String oneValue="";
								for(int l = 3;l<lastColumn;l++){
								int sencondColumn = firstColumn+1;
								if(sencondColumn >= firstColumn &&sencondColumn <= lastColumn) {
									org.apache.poi.ss.usermodel.Row SRow = sheet.getRow((sencondColumn));
									...
									ScellValue = SCell.getStringCellValue();
										...
										int ThrColumn = firstColumn+3;
										org.apache.poi.ss.usermodel.Row TRow = sheet.getRow(ThrColumn);
